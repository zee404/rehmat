...
class _MapViewState extends State<MapView> {
  ...
  populateClients(List<UserDetails> allUsers, List<BloodRequest> allRequest) {
    if (allRequest != null) {
      if (allRequest.isNotEmpty) {
        for (int i = 0; i < allRequest.length; ++i) {
          if (allRequest[i].uid != BaseViewModel().getUser.uid) {
            initMarker(allRequest[i].uid,
                request: allRequest[i], allUsers: allUsers);
          }
        }
      }
    } else {
